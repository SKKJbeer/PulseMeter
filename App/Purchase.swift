...
protocol PurchaseGateway: Sendable {
    ...
    /// Der Preis, wie er dem Nutzer gezeigt wird.
    ///
    /// Kommt später aus StoreKit und damit in der Währung des Ladens, in dem
    /// der Nutzer steht. Fest verdrahtet wäre er in der Schweiz falsch.
    var displayPrice: String? { get }

    func purchase() async throws -> Entitlement
...
struct UnavailablePurchaseGateway: PurchaseGateway {
    ...
    func purchase() async throws -> Entitlement { throw Unavailable.notYetInTheStore }
...
final class Purchase {
    ...
    init(defaults: UserDefaults = .standard, gateway: PurchaseGateway = UnavailablePurchaseGateway()) {
        ...
        if arguments.contains("-pulse-frei") {
            ...
            entitlement = Entitlement(rawValue: defaults.string(forKey: Self.storageKey) ?? "") ?? .free
        }
    }

    /// Ob eine Leistung offensteht — die Frage, die jede Ansicht stellt.
    func allows(_ feature: ProFeature) -> Bool { policy.allows(feature) }

    func canAddMeter(existingCount: Int) -> Bool { policy.canAddMeter(existingCount: existingCount) }

    func buy() async {
        await run { try await gateway.purchase() }
    }
